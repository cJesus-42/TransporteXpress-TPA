...
class MenuAdmin(Conductores, Camiones):
    ...
    def mostrarMenuHorizontalCamiones(self):
        ...
        botones = [
            ("Agregar", self.formularioAgregarCamion),
            ("Editar", self.accionEditar),
            ("Eliminar", self.accionEliminar),
            ("Volver", self.volverBotones)
        ]
...
############################################################################################
     ...
     # Contenedor para la lista de camión debajo de la barra
        self.contenedorLista = Frame(self.root)
        self.contenedorLista.pack(fill=BOTH, expand=True, padx=20, pady=20)

        # Crear el Treeview con columnas para cada dato
        columnas = ("Patente", "Marca", "Teléfono", "Correo", "Dirección", "Fecha Nacimiento", "Fecha Ingreso")
        self.treeamiones = ttk.Treeview(self.contenedorLista, columns=columnas, show='headings')
    ...
    def formularioAgregarCamion(self):
        self.limpiarWidgets()
        contenedorFormulario = Frame(self.root, bg="orange")
        contenedorFormulario.pack(padx=20, pady=20)
        ...
        Entry(contenedorFormulario, textvariable=self.idCamion).grid(row=0, column=1, pady=5)

        Label(contenedorFormulario, text="Marca:", bg="orange").grid(row=1, column=0, sticky=E, pady=5)
        Entry(contenedorFormulario, textvariable=self.marcaCamion).grid(row=1, column=1, pady=5)
        ...
        # Botón para guardar
        Button(contenedorFormulario, text="Guardar", command=self.formularioGuardarCamion).grid(row=7, column=0, columnspan=2, pady=15)
        Button(contenedorFormulario, text="Volver", command=self.volverBotones).grid(row=7, column=1, columnspan=2, pady=15)

    def formularioGuardarCamion(self):
        # Obtiene los valores
        idCamion = self.idCamion.get().strip()
        marcaCamion = self.marcaCamion.get().strip()
        telefono = self.telefono.get().strip()
        correo = self.correo.get().strip()
        direccion = self.direccion.get().strip()
        nacimiento = self.nacimiento.get().strip()
        ...
        # Validaciones básicas
        if not idCamion or not marcaCamion:
            messagebox.showerror("Error", "Debe ingresar al menos la Patente y Marca del camión.")
            return

        # Validar que la Patente tenga a lo más 6 dígitos
        if len(idCamion.replace('-', '').replace('.', '')) <= 6:
            messagebox.showerror("Error", "La Patente debe tener a lo más dígitos.")
            return

        # Cargar camiones existentes
        camiones = self.cargarCamiones()

        # Verificar si ya existe el camión por la patente
        for c in camiones:
            if c['Patente'] == idCamion:
                messagebox.showerror("Error", "Ya existe una patente con un camión asociado.")
                return
        ...
        def listaCamiones (self):
            ...
            conductores = self.cargarCamiones()  # Método que carga conductores desde JSON
            ...
            for camion in camiones:
                self.treeConductores.insert('', 'end', values=(
                    ...
                    camion.get("Marca", ""),
                    camion.get("Teléfono", ""),
                    camion.get("Correo Electrónico", ""),
                    camion.get("Dirección", ""),
                    camion.get("Fecha Nacimiento", ""),
                    camion.get("Fecha Ingreso", ""),
                ))
    ...
    def accionEliminar(self):
        ...
        # Guarda la lista actualizada
        self.guardarConductores(nuevos_camiones)

        # Refresca la tabla
        self.listaConductores()
        messagebox.showinfo("Éxito", f"Conductor con RUT {patente_a_eliminar} eliminado correctamente.")
